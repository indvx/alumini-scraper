...
<div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 md:p-8 shadow-sm">
    ...
    <div class="mt-6 p-4 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300 text-sm flex items-center justify-between">
        <div>
            <strong>Search Completed for "{{ is_array($searchResult['search']) ? $searchResult['search']['query'] : $searchResult['search']->query }}":</strong>
            Found <strong>{{ count($searchResult['institutions'] ?? []) }}</strong> institutions ({{ is_array($searchResult['search']) ? $searchResult['search']['display_name'] : $searchResult['search']->display_name }}).
        </div>
        <span class="px-2.5 py-1 rounded-md text-xs font-bold {{ !empty($searchResult['cached']) ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800' }}">
