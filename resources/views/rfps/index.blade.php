    ...
    <div class="flex h-full w-full flex-1 flex-col gap-6">
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <button type="button"
                ...
                class="flex items-center gap-2 px-3.5 py-2 bg-rose-600 hover:bg-rose-700 rounded-xl text-white text-xs font-bold transition-all shadow-sm">
        ...
        <!-- Scrape RFPs Form (Collapsible) -->
        <div id="scrape-rfps-form-container"
            class="hidden p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4 transition-all">
            <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                ...
                <span class="text-[11px] text-slate-400">Specify institution and platform to trigger scraper</span>
            </div>
            ...
            <form action="{{ route('rfps.scrape') }}" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    <div>
                        ...
                        <select name="platform_name" required
                            class="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white text-xs focus:outline-none focus:ring-2 focus:ring-rose-500">
                            <option value="Bonfire" selected>Bonfire</option>
                ...
                <div class="flex justify-end gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                    <button type="button"
                        ...
                        class="px-3.5 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-semibold">
                        Cancel
                    </button>
                    <button type="submit"
                        class="px-5 py-2 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold shadow-sm transition-all flex items-center gap-1.5">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M13 10V3L4 14h7v7l9-11h-7z" />
                        </svg>
        ...
        <!-- Filter Bar -->
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-2 shadow-sm">
            <form action="{{ route('rfps.index') }}" method="GET"
                ...
                <div>
                    ...
                    <select name="status"
                        ...
                        <option value="awarded" {{ ($filters['status'] ?? '') == 'awarded' ? 'selected' : '' }}>Awarded
                        </option>
                ...
                <div class="flex items-end gap-2">
                    <button type="submit"
                        class="w-full py-2 px-4 rounded-xl bg-slate-900 hover:bg-slate-800 dark:bg-slate-700 dark:hover:bg-slate-600 text-white text-xs font-bold transition-colors">
                        Filter
                    </button>
                    <a href="{{ route('rfps.index') }}"
                        class="py-2 px-3 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-semibold transition-colors">
                        Reset
                    </a>
                </div>
            </form>
        ...
        <!-- RFP Items Container -->
        @if ($rfps->isNotEmpty())
            <div class="space-y-1 max-h-[500px] overflow-y-auto pr-1.5">
                @foreach ($rfps as $rfp)
                    @php
                        $statusLower = strtolower((string) $rfp->status);
                    @endphp
                    <div
                        class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-3 shadow-xs hover:border-slate-400 dark:hover:border-slate-500 transition-all space-y-3">
                        <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                            <div class="space-y-1.5 flex-1">
                                ...
                                <!-- Title -->
                                <h3 class="text-base md:text-lg font-extrabold text-slate-900 dark:text-white leading-snug">
                        ...
                        <div
                            ...
                            @if ($rfp->opportunity_url || $rfp->portal_url)
                                <a href="{{ $rfp->opportunity_url ?: $rfp->portal_url }}" target="_blank"
                                    rel="noopener"
                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold shadow-xs transition-all w-fit">
        ...
        @else
            <div
                ...
                <a href="{{ route('rfps.index') }}"
                    class="inline-block px-4 py-2 rounded-xl bg-slate-900 dark:bg-slate-700 text-white text-xs font-bold">
                    Clear Filters
                </a>
            </div>
        @endif
