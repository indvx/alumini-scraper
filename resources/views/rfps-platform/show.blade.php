        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 shadow-sm space-y-6">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-6">
                        <div>
                            <div class="flex items-center gap-2 mb-2">
                                <span class="px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider bg-rose-100 text-rose-800 dark:bg-rose-950/60 dark:text-rose-300">
                                ...
                                @else
                                    <span class="px-2.5 py-0.5 rounded-full text-xs font-bold bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400">
                                        Inactive
                                    </span>
                                @endif
                            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <!-- Information Block -->
                        <div class="space-y-4">
                            <h3 class="text-xs font-bold uppercase tracking-wider text-rose-600 dark:text-rose-400">Platform Specs</h3>
                                        ...
                                        @if ($platform->institution_type)
                                            <div class="flex flex-wrap gap-1 justify-end max-w-xs">
                                                @foreach (array_filter(array_map('trim', explode(',', $platform->institution_type))) as $t)
                                                    <span class="px-2 py-0.5 text-xs rounded bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-normal">
                                                        {{ $t }}
                                                    </span>
                                                @endforeach
                                            </div>
                        ...
                        <div class="space-y-4">
                            <h3 class="text-xs font-bold uppercase tracking-wider text-rose-600 dark:text-rose-400">Location &amp; Activity</h3>

                            <dl class="space-y-3 text-sm">
                                <div class="flex justify-between py-2 border-b border-slate-100 dark:border-slate-800">
                                    <dt class="text-slate-500 dark:text-slate-400 font-medium">City</dt>
                                    <dd class="font-semibold text-slate-900 dark:text-white">{{ $platform->city ?: 'N/A' }}</dd>
                                </div>
                                <div class="flex justify-between py-2 border-b border-slate-100 dark:border-slate-800">
                                    <dt class="text-slate-500 dark:text-slate-400 font-medium">State / Province</dt>
                                    <dd class="font-semibold text-slate-900 dark:text-white">{{ $platform->state ?: 'N/A' }}</dd>
                                </div>
                                <div class="flex justify-between py-2 border-b border-slate-100 dark:border-slate-800">
                                    <dt class="text-slate-500 dark:text-slate-400 font-medium">Country</dt>
                                    <dd class="font-semibold text-slate-900 dark:text-white">{{ $platform->country ?: 'N/A' }}</dd>
                                </div>
                                <div class="flex justify-between py-2 border-b border-slate-100 dark:border-slate-800">
                    ...
                    <div class="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs text-slate-400">
                        ...
                        <span>Last Updated {{ $platform->updated_at ? $platform->updated_at->format('M d, Y H:i') : 'N/A' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm space-y-4">
                    <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                        <div>
                            <h3 class="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
                                ...
                                <span class="px-2 py-0.5 rounded-full text-xs font-bold bg-indigo-100 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300">
                        ...
                        <button type="button" onclick="document.getElementById('add-institution-relation-form').classList.toggle('hidden');"
                    ...
                    <div id="add-institution-relation-form" class="hidden p-4 rounded-xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 space-y-3 transition-all">
                        <h4 class="text-xs font-bold uppercase tracking-wider text-rose-600 dark:text-rose-400">
                            Add Institution Relation
                        </h4>
                        <form action="{{ route('rfps-platform.institutions.store', $platform) }}" method="POST" class="space-y-3">
                            @csrf
                            <div>
                                <label for="institution_id" class="block text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1">
                                ...
                                <select id="institution_id" name="institution_id" required
                                    ...
                                    @foreach ($allInstitutions as $inst)
                                        <option value="{{ $inst->id }}">{{ $inst->name }} ({{ $inst->city ?: ($inst->state ?: $inst->type) }})</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <div>
                                    <label for="rel_confidence" class="block text-[11px] font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1">
                                        Confidence (%)
                                    </label>
                                    <input type="number" min="0" max="100" id="rel_confidence" name="confidence" value="98"
                                ...
                                <div>
                                    <label for="inst_rel_status" class="block text-[11px] font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1">
                                        Status
                                    </label>
                            ...
                            <div>
                                <label for="inst_discovery_method" class="block text-[11px] font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1">
                                    Discovery Method
                                </label>
                                <input type="text" id="inst_discovery_method" name="discovery_method" value="AI + Web Search"
                                    placeholder="e.g. AI + Web Search"
                                    class="w-full px-2.5 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white text-xs focus:outline-none focus:ring-2 focus:ring-rose-500">
                            </div>

                            <div>
                                <label for="inst_source_title" class="block text-[11px] font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1">
                                    Source Title
                                </label>
                                <input type="text" id="inst_source_title" name="source_title" value="University procurement page"
                                    placeholder="e.g. University procurement page"
                                    class="w-full px-2.5 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white text-xs focus:outline-none focus:ring-2 focus:ring-rose-500">
                            </div>

                            <div>
                                <label for="inst_source_url" class="block text-[11px] font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1">
                                    Evidence URL (Source Link)
                                </label>
                                <input type="url" id="inst_source_url" name="source_url" placeholder="https://procurement.example.edu/bids"
                                    class="w-full px-2.5 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white text-xs focus:outline-none focus:ring-2 focus:ring-rose-500">
                            </div>

                            <div class="grid grid-cols-2 gap-2">
                                <div>
                                    <label for="inst_first_verified_at" class="block text-[11px] font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1">
                                        First Verified
                                    </label>
                                    <input type="date" id="inst_first_verified_at" name="first_verified_at" value="{{ date('Y-m-d') }}"
                                        class="w-full px-2.5 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white text-xs focus:outline-none focus:ring-2 focus:ring-rose-500">
                                </div>

                                <div>
                                    <label for="inst_last_verified_at" class="block text-[11px] font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1">
                                        Last Verified
                                    </label>
                                    <input type="date" id="inst_last_verified_at" name="last_verified_at" value="{{ date('Y-m-d') }}"
                                        class="w-full px-2.5 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white text-xs focus:outline-none focus:ring-2 focus:ring-rose-500">
                                </div>
                            </div>

                            <div>
                                <label for="inst_notes" class="block text-[11px] font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1">
                                    Notes
                                </label>
                                <textarea id="inst_notes" name="notes" rows="2" placeholder="e.g. Formal bid opportunities are posted through Bonfire."
                            ...
                            <div class="flex justify-end gap-2 pt-2 border-t border-slate-200 dark:border-slate-700">
                                <button type="button" onclick="document.getElementById('add-institution-relation-form').classList.add('hidden');"
                            ...
                            @foreach ($platform->institutions as $inst)
                                <div class="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800 space-y-2.5">
                                    <div class="flex items-start justify-between gap-2 border-b border-slate-100 dark:border-slate-800 pb-2">
                                        <div>
                                            <a href="{{ route('institutions.show', $inst) }}" class="text-sm font-bold text-slate-900 dark:text-white hover:text-rose-600 transition-colors">
                                                {{ $inst->name }}
                                            </a>
                                            <p class="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
                                                <span class="font-semibold text-slate-700 dark:text-slate-300">{{ $inst->name }}</span> uses <span class="font-semibold text-slate-700 dark:text-slate-300">{{ $platform->name }}</span>
                                            </p>
                                        </div>

                                        <form action="{{ route('rfps-platform.institutions.destroy', [$platform, $inst]) }}" method="POST"
                                    ...
                                    <div class="flex items-center gap-1.5 flex-wrap text-[10px]">
                                        <span class="px-2 py-0.5 font-bold uppercase rounded bg-slate-200 text-slate-700 dark:bg-slate-700 dark:text-slate-300">
                                            {{ $inst->type }}
                                        </span>
                                    ...
                                    <div class="space-y-1 text-xs">
                                        @if ($inst->pivot->discovery_method)
                                            <div class="flex justify-between">
                                                ...
                                                <span class="font-medium text-slate-700 dark:text-slate-300">{{ $inst->pivot->discovery_method }}</span>
                                            </div>
                                        @endif
                                        ...
                                        @if ($inst->pivot->source_title)
                                            <div class="flex justify-between">
                                                ...
                                                <span class="font-medium text-slate-700 dark:text-slate-300">{{ $inst->pivot->source_title }}</span>
                                            </div>
                                        @endif
                                    </div>
