    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                ...
                <form action="{{ route('rfps-platform.destroy', $platform) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete {{ addslashes($platform->name) }}?');" class="inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
                ...
                <div class="space-y-4">
                    ...
                    <dl class="space-y-3 text-sm">
                        <div class="flex justify-between py-2 border-b border-slate-100 dark:border-slate-800">
                            ...
                            <dd class="font-semibold text-slate-900 dark:text-white">{{ $platform->institution_type ?: 'Not Specified' }}</dd>
                        </div>
                        <div class="flex justify-between py-2 border-b border-slate-100 dark:border-slate-800">
                            <dt class="text-slate-500 dark:text-slate-400 font-medium">Coverage Area</dt>
                            <dd class="font-semibold text-slate-900 dark:text-white">{{ $platform->coverage ?: 'Not Specified' }}</dd>
                        </div>
                        <div class="flex justify-between py-2 border-b border-slate-100 dark:border-slate-800">
                            <dt class="text-slate-500 dark:text-slate-400 font-medium">Public Access</dt>
                            <dd class="font-semibold text-slate-900 dark:text-white">{{ $platform->is_public ? 'Yes (Public)' : 'No (Private)' }}</dd>
                        </div>
                        <div class="flex justify-between py-2 border-b border-slate-100 dark:border-slate-800">
                            <dt class="text-slate-500 dark:text-slate-400 font-medium">Requires Login</dt>
                            <dd class="font-semibold text-slate-900 dark:text-white">{{ $platform->requires_login ? 'Yes' : 'No' }}</dd>
                        </div>
                    </dl>
                </div>
