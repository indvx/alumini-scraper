<x-layouts.app title="RFPs Platforms">
    <div class="flex h-full w-full flex-1 flex-col gap-6">
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-3 shadow-sm">
            <form action="{{ route('rfps-platform.index') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-1">
                <div>
                    <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">Search</label>
                    <input type="text" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Name, domain, city..."
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">Platform Type</label>
                    <select name="platform_type" class="w-full px-3 py-2 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-rose-500">
                        <option value="all">All Types</option>
                        @foreach($platformTypes as $type)
                        <option value="{{ $type }}" {{ ($filters['platform_type'] ?? '') == $type ? 'selected' : '' }}>{{ ucfirst($type) }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="country" class="w-full px-3 py-2 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-rose-500">
                ...
                <div>
                    ...
                    <select name="status" class="w-full px-3 py-2 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-rose-500">
                ...
                <div class="flex items-end gap-2">
                    <button type="submit" class="w-full py-2 px-4 rounded-lg bg-slate-900 hover:bg-slate-800 dark:bg-slate-700 dark:hover:bg-slate-600 text-white text-sm font-semibold transition-colors">
                        Filter
                    </button>
                    <a href="{{ route('rfps-platform.index') }}" class="py-2 px-4 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-sm font-semibold transition-colors">
                        Reset
                    </a>
                </div>
            </form>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="text-sm divide-y divide-slate-100 dark:divide-slate-800">
                        ...
                        <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                            <td class="py-4 px-4">
                                <div class="text-sm text-slate-500 dark:text-slate-400">{{ $p->id }}</div>
                            </td>
                            <td class="py-4 px-4">
                                <div class="font-bold text-slate-900 dark:text-white">
                                    <a href="{{ route('rfps-platform.show', $p) }}" class="hover:text-rose-600 transition-colors">
                                        {{ $p->name }}
                                    </a>
                                </div>
                                @if($p->domain || $p->url)
                                <div class="text-xs text-slate-400 dark:text-slate-500 font-mono mt-0.5">
                                    @if($p->url)
                                    <a href="{{ $p->url }}" target="_blank" rel="noopener" class="hover:underline flex items-center gap-1">
                            ...
                            <td class="py-4 px-4">
                                <div class="flex flex-wrap gap-1">
                                    @if($p->is_public)
                                    <span class="px-2 py-0.5 text-[11px] font-bold rounded-full bg-emerald-100 text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-300">
                                        Public
                                    </span>
                                    @else
                                    <span class="px-2 py-0.5 text-[11px] font-bold rounded-full bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-400">
                                        Private
                                    </span>
                                    @endif
                                    @if($p->requires_login)
                                    <span class="px-2 py-0.5 text-[11px] font-bold rounded-full bg-amber-100 text-amber-800 dark:bg-amber-950/60 dark:text-amber-300">
                                        Login Required
                                    </span>
                                    @endif
                                </div>
                            ...
                            <td class="py-4 px-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('rfps-platform.show', $p) }}"
                                        class="p-1.5 text-slate-500 hover:text-rose-600 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors" title="View Details">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <a href="{{ route('rfps-platform.edit', $p) }}"
                                        class="p-1.5 text-slate-500 hover:text-indigo-600 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                        </svg>
                                    </a>
                                    <form action="{{ route('rfps-platform.destroy', $p) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete {{ addslashes($p->name) }}?');" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="p-1.5 text-slate-500 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/50 rounded-lg transition-colors" title="Delete">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="p-12 text-center">
            ...
            <div class="mt-4">
                <a href="{{ route('rfps-platform.create') }}" class="px-4 py-2 rounded-lg bg-rose-600 text-white text-xs font-semibold inline-flex items-center gap-1.5 shadow-sm">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                    </svg>
                    Add Platform
    ...
    <!-- AI Search Modal -->
    <div id="aiSearchModal" class="fixed inset-0 z-50 hidden flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 max-w-xl w-full shadow-2xl relative space-y-6">

            <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl bg-gradient-to-tr from-purple-600 via-indigo-600 to-rose-600 flex items-center justify-center text-white text-lg font-black shadow-md">
                        ✨
                    </div>
                    <div>
                        <h3 class="text-lg font-black text-slate-900 dark:text-white">Find Platforms by Location (AI)</h3>
                        <p class="text-xs text-slate-500 dark:text-slate-400">Enter a location (state, city, or region) and AI will find &amp; generate platforms for that area.</p>
                    </div>
                </div>
                <button type="button" onclick="closeAiSearchModal()" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-1.5 rounded-lg transition-colors">
            ...
            <form action="{{ route('rfps-platform.ai-search') }}" method="GET" class="space-y-5">
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-2">Target Location</label>
                ...
                <div>
                    <p class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">Suggested Location Examples</p>
                ...
                <div class="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end gap-3">
                    <button type="button" onclick="closeAiSearchModal()" class="px-5 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-sm font-semibold transition-colors">
                        Cancel
                    </button>
                    <button type="submit" class="px-6 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 via-indigo-600 to-rose-600 hover:from-purple-700 hover:to-rose-700 text-white text-sm font-bold shadow-md shadow-purple-600/20 transition-all flex items-center gap-2">
                        <span>✨ Find Platforms by Location</span>
                    </button>
                </div>
            </form>
        </div>
