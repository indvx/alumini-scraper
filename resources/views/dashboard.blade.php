    ...
    <div class="space-y-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div
                ...
                <span class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 block">Total
                    Institutions</span>
                <span
                    class="text-3xl font-extrabold text-slate-900 dark:text-white mt-2 block">{{ number_format($totalInstitutions) }}</span>
            </div>

            <div
                class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
                <span
                    class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 block">Locations
                    Searched</span>
                <span
                    class="text-3xl font-extrabold text-slate-900 dark:text-white mt-2 block">{{ number_format($totalSearches) }}</span>
            </div>

            <div
                class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
                <span
                    class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 block">Schools
                    & Colleges</span>
            ...
            <div
                class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
                <span
                    class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 block">Universities</span>
                    ...
                    <div class="divide-y divide-slate-100 dark:divide-slate-800 h-[30vh] overflow-y-auto">
                        @foreach ($recentSearches as $s)
                            <div class="py-3 flex items-center justify-between">
                                <div>
                                    <strong
                                        class="text-sm text-slate-900 dark:text-white block">{{ $s->query }}</strong>
                                    <span
                                        class="text-xs text-slate-500 dark:text-slate-400 block truncate max-w-sm">{{ $s->display_name }}</span>
                                </div>
                                <span
                                    class="text-xs font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-md">
                            ...
                            <div class="py-3 flex items-center justify-between">
                                <div>
                                    <a href="{{ route('institutions.show', $inst->id) }}"
                                        class="text-sm font-bold text-slate-900 dark:text-white hover:text-rose-600">
                                        {{ $inst->name }}
                                    </a>
                                    <span
