    ...
    <div class="max-w-3xl mx-auto space-y-3">
        ...
        <div class="flex items-center justify-between">
            <a href="{{ route('institutions.show', $institution) }}"
                class="inline-flex items-center gap-1 text-xs font-bold tracking-wider text-rose-600 dark:text-rose-400 hover:underline">
        ...
        <div
            class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-1 sm:p-5 shadow-sm">
            <div class="border-b border-slate-100 dark:border-slate-800 pb-2 mb-2">
                <h1 class="text-xl font-black text-slate-900 dark:text-white">Edit Institution: {{ $institution->name }}
                </h1>
            ...
            <form action="{{ route('institutions.update', $institution) }}" method="POST" class="space-y-2">
                    ...
                    <div>
                        ...
                        <select id="type" name="type" required
                            ...
                            <option value="college"
                                {{ old('type', $institution->type) === 'college' ? 'selected' : '' }}>College</option>
                            <option value="university"
                                {{ old('type', $institution->type) === 'university' ? 'selected' : '' }}>University
                            </option>
                            <option value="kindergarten"
                                {{ old('type', $institution->type) === 'kindergarten' ? 'selected' : '' }}>Kindergarten
                            </option>
                            <option value="other" {{ old('type', $institution->type) === 'other' ? 'selected' : '' }}>
                                Other</option>
                        </select>
                        @error('type')
                            <p class="text-xs text-rose-500 mt-1 font-medium">{{ $message }}</p>
                ...
                <div class="space-y-4 pt-2 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <div>
                        <label for="address"
                            class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">Street
                            Address</label>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        ...
                        <div>
                            <label for="postcode"
                                class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">Postcode
                                / ZIP</label>
                ...
                <!-- Contact & Web Section -->
                <div class="space-y-4 pt-2 border-t border-slate-100 dark:border-slate-800">
                    <h2 class="text-xs font-bold uppercase tracking-wider text-rose-600 dark:text-rose-400">Contact
                        &amp; Website</h2>

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label for="phone"
                                class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">Phone
                                Number</label>
                ...
                <!-- Action Buttons -->
                <div
                    class="pt-6 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3">
                    <a href="{{ route('institutions.show', $institution) }}"
            ...
            <form id="delete-institution-form" action="{{ route('institutions.destroy', $institution) }}"
