    ...
    <div class="flex h-full w-full flex-1 flex-col gap-6">
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm">
            <form action="{{ route('institutions.index') }}" method="GET"
                ...
                <div>
                    ...
                    <select name="type"
                        ...
                        <option value="college" {{ ($filters['type'] ?? '') == 'college' ? 'selected' : '' }}>
                            College</option>
                        <option value="kindergarten" {{ ($filters['type'] ?? '') == 'kindergarten' ? 'selected' : '' }}>
                            Kindergarten</option>
                    </select>
                </div>
                <div>
                ...
                <div class="flex items-end gap-2">
                    <button type="submit"
                        class="w-full bg-rose-600 hover:bg-rose-700 text-white font-semibold py-2 px-4 rounded-lg text-sm transition-all shadow-sm">
                        Filter
                    </button>
                    <a href="{{ route('institutions.index') }}"
                        class="py-2 px-4 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-sm font-semibold transition-colors">Reset</a>
                </div>
            </form>
        </div>
        <div>
            <p class="text-xs text-slate-500 dark:text-slate-400">Page {{ $institutions->currentPage() }} of
                {{ $institutions->lastPage() }}</p>
        </div>
        <!-- Grid of Cards -->
        @if ($institutions->count() > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 h-[54vh] overflow-y-scroll">
                @foreach ($institutions as $inst)
                    <div id="{{ $inst->id }}"
                        class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-3 shadow-sm hover:border-slate-300 dark:hover:border-slate-900 flex flex-col justify-between space-y-1 transition-all">
                        <div class="space-y-2">
                            <div class="flex items-center justify-between">
                                <span
                                    class="px-2.5 py-0.5 rounded text-xs font-extrabold uppercase tracking-wider 
                                ...
                                @if ($inst->osm_id)
                                    <span class="text-[10px] text-slate-400 font-mono">OSM
                                        #{{ $inst->osm_id }}</span>
                                @endif
                            </div>
                            <div class="flex items-center justify-between">
                        ...
                        <div
                            ...
                            <div class="flex items-center space-x-2 text-slate-500 dark:text-slate-400">
                                @if ($inst->phone)
                                    <a href="tel:{{ $inst->phone }}" class="hover:text-rose-600"
                                        title="{{ $inst->phone }}">
                                ...
                                @if ($inst->website)
                                    <a href="{{ $inst->website }}" target="_blank" rel="noopener"
                                        class="hover:text-rose-600" title="{{ $inst->website }}">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        @else
            <div
                ...
                <p class="text-base font-semibold text-slate-700 dark:text-slate-300">No institutions match your
                    search or filter.</p>
