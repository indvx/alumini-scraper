            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-6">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-xs text-slate-500 dark:text-slate-400 font-mono bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-md">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-slate-100 dark:border-slate-800 text-sm">
                        ...
                        <div class="p-4 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-100 dark:border-slate-700">
                            ...
                            <span class="text-slate-900 dark:text-white font-mono text-xs mt-0.5 block">
                                ...
                                {{ number_format($institution->latitude, 6) }}, {{ number_format($institution->longitude, 6) }}
                                @else
                                N/A
                                @endif
                            </span>
                ...
                <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm space-y-3">
                    ...
                    <div id="single-institution-map" class="w-full rounded-xl border border-slate-200 dark:border-slate-700 z-10" style="min-height: 340px; height: 340px;"></div>
                </div>
                @endif
                    ...
                    <div class="space-y-3">
                        ...
                        <a href="{{ route('public.institutions.show', $nearby->id) }}"
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-[10px] font-bold text-slate-600 dark:text-slate-300 bg-slate-200 dark:bg-slate-700 px-2 py-0.5 rounded-md">
                                    {{ number_format($nearby->distance, 1) }} km
                                </span>
                            </div>
                            <span class="text-xs text-slate-500 dark:text-slate-400 block capitalize">{{ $nearby->type }} &bull; {{ $nearby->city ?: $nearby->state }}</span>
                        </a>
                        @endforeach
                    </div>
                </div>
